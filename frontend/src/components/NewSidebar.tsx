...
import { USER_ME_ENDPOINT } from '../lib/api';
...
const NewSidebar = ({ navItems, activeTab, setActiveTab }) => {
    ...
    const { data: user, isLoading } = useQuery({ queryKey: ['userMe'], queryFn: fetchUserMe });
            ...
            <div className="relative z-10 flex flex-col h-full">
                ...
                <div className={`flex items-center gap-3 px-6 py-4 h-20`}>
                    {isLoading ? (
                        ...
                        <img src={user?.avatar_url || "https://github.com/github.png"} alt="User Avatar" className="w-11 h-11 rounded-full border-2 border-white/50" />
                    )}
                    <AnimatePresence>
                        {!isCollapsed &&
                            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }} exit={{ opacity: 0, x: -10 }} >
                                {isLoading ? (
                                    ...
                                    <p className="font-bold text-sm text-gray-800 dark:text-white truncate max-w-[120px]">{user?.name || user?.username || "Bot User"}</p>
                                )}
